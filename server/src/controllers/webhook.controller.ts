...
export const handleBaasCallback = async (
  ...
  try {
    ...
    if (payload.event === "bot.completed") {
      // 🌟 FIX 1: 'extra' is OUTSIDE of 'data'. Also 'mp4' is actually called 'video'.
      const { bot_id, transcription, video } = payload.data;
      ...
      try {
        let realTranscriptText: any = "";

        // 🌟 FIX 2: Handle 'null' transcriptions safely so axios doesn't crash
        if (transcription) {
          console.log("📥 Downloading real transcript data from AWS S3...");
          const s3Response = await axios.get(transcription);
          realTranscriptText = s3Response.data;
        } else {
          console.warn(
            "⚠️ [WARNING] Meeting BaaS returned 'null' for transcription.",
          );
          realTranscriptText = [
            {
              speaker: "System",
              words: [
                {
                  text: "No transcription data was generated by the provider for this meeting.",
                },
              ],
            },
          ];
        ...
        const savedRecord = await MeetingRecordModel.create({
          ...
          videoUrl: video, // 🌟 Changed from mp4
